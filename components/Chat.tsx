...
export default function Chat({ sendMessage }: { sendMessage: (message: string) => void }) {
  ...
  const handleOptionClick = (option: string) => {
    // Add the option to the selectedOptions array if it's not already selected
    if (!selectedOptions.includes(option)) {
      const newSelectedOptions = [...selectedOptions, option];
      setSelectedOptions(newSelectedOptions);

      // Set userMessage to be the joined selected options
      ...
      // Update userMessage with the remaining selected options
      setUserMessage(newSelectedOptions.join(", "));
    }
  };
